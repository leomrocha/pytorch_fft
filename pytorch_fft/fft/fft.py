...
def _fft(X_re, X_im, f, rank):
    ...
    if not(X_re.dim() >= rank and X_im.dim() >= rank):
        raise ValueError("Inputs must have at least {} dimensions.".format(rank))
...
def _rfft(X, f, rank):
    if not(X.dim() >= rank):
        raise ValueError("Input must have at least {} dimensions.".format(rank))
    if not(X.is_cuda):
        raise ValueError("Input must be a CUDA tensor.")
